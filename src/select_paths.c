									...
									t_room *end_room_ptr, t_validity *validity)
{
	...
	best_room = NULL;
	*validity = no_room;
	while (adj_room_elem && *validity != many_alternatives)
	{
...
static t_valid_path		*find_shortest_path(t_input *input)
{
	...
	validity = create_path(input, path, adj_room_elem);
	if (validity != no_room)
	{
...
int						select_paths(t_input *input, t_report *report)
{
	...
	while (valid_path && report->number_of_paths < num_of_paths)
	{
		ft_lstadd(report->lst_of_valid_paths, ft_lstnew(&valid_path,
														sizeof(valid_path)));
		report->number_of_paths++;
		if (report->number_of_paths < num_of_paths)
			valid_path = find_shortest_path(input);
	}
