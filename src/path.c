...
static void		select_paths_2(t_output *output, t_list **path_lst,
								size_t *merged_room_vector, size_t barach_id, size_t *num_of_paths_in_branch_array)
{
	t_list			*elem;
	size_t			i;
	t_valid_path	*valid_path;
...
//	free(i_array);
	path_lst = NULL;
	barach_id = 0;
	select_paths_2(output, &path_lst, merged_room_vector, barach_id, num_of_paths_in_branch_array);
	return ;
}
